...
- A column is left alone only when its measured CONTENT has the shape of an identifier, never on the strength of its name alone. In an import-ready export, `partner_id` holds the relation's display name and `display_name` is the data itself, so deciding on the label copied the most identifying columns of the file verbatim while announcing them as protected
...
- Une colonne n'est laissée intacte que si son CONTENU mesuré a la forme d'un identifiant, jamais sur la seule foi de son nom. Dans un export import-compatible, `partner_id` porte le nom affiché de la relation et `display_name` est la donnée elle-même : décider sur l'étiquette recopiait textuellement les colonnes les plus identifiantes du fichier en les annonçant comme protégées
