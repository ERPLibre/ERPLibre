...
- `Execute › Deploy › Set-OPS` drives the Set-OPS engine without copying it. The engine is opt-in: `manifest/git_manifest_setops.xml` declares it at a pinned commit under `private/repo/`, which git ignores, and `./script/manifest/update_manifest_local_setops.sh` fetches it alone through Google Repo. Its guards all run before any side effect and name every refusal in one run: `repo` missing, a detached HEAD that no branch contains, and, in the engine's place, any folder Google Repo did not lay out — even at a path `.repo/project.list` lists — with the `mv` that sets it aside. `make repo_configure_group_code_generator` keeps the engine of a station that opted in (group `setops`), since a sync that leaves it out deletes its tree. Its first screen, the state of the integration, reads ten lines — platform, manifest, private location, Google Repo, engine, Ansible, ecosystem, site, vault key, tools — each with its source and the gesture that settles it, and writes nothing. See [doc/SETOPS.md](doc/SETOPS.md)
...
- `Execute › Deploy › Set-OPS` pilote le moteur Set-OPS sans le recopier. Le moteur est sur demande : `manifest/git_manifest_setops.xml` le déclare à un commit épinglé sous `private/repo/`, que git ignore, et `./script/manifest/update_manifest_local_setops.sh` le rapatrie seul par Google Repo. Ses gardes passent toutes avant tout effet de bord et nomment chaque refus en une exécution : `repo` absent, un HEAD détaché qu'aucune branche ne contient, et, à la place du moteur, tout dossier que Google Repo n'a pas posé — même à un chemin que `.repo/project.list` porte —, avec le `mv` qui le met de côté. `make repo_configure_group_code_generator` garde le moteur d'un poste qui l'a rapatrié (groupe `setops`), puisqu'un sync qui l'écarte supprime son arbre. Son premier écran, l'état de l'intégration, lit dix lignes — plateforme, manifeste, emplacement privé, Google Repo, moteur, Ansible, écosystème, site, clé de voûte, outils — chacune avec sa source et le geste qui la règle, et n'écrit rien. Voir [doc/SETOPS.fr.md](doc/SETOPS.fr.md)
...
- The Proxmox orphan-volume cleanup judges each volume against the VMs of the WHOLE cluster (`pvesh get /cluster/resources`), joined to those of `qm list`. A shared storage also carries the disks of the other nodes' VMs, which `qm list` does not see, and they were offered for deletion. When either list cannot be read, nothing is offered, and the refusal shows the command actually sent — sudo, jump host and port included — its exit code and the last lines of its output. The volume list refuses the same way, and shows why, when `pvesm status` or the last storage listed fails; a storage that fails before the last only drops its own volumes, which are never offered. A failing `pvesm status` no longer reads as « Nothing orphaned ». The cluster list stays readable when an sshd banner or an ssh or sudo warning follows its JSON document
...
- Le nettoyage des volumes orphelins de Proxmox juge chaque volume contre les VM de TOUTE la grappe (`pvesh get /cluster/resources`), jointes à celles de `qm list`. Un stockage partagé porte aussi les disques des VM des autres nœuds, que `qm list` ne voit pas, et ils étaient offerts à la suppression. Quand l'une des deux listes ne se lit pas, rien n'est offert, et le refus montre la commande réellement envoyée — sudo, rebond et port compris —, son code de retour et les dernières lignes de sa sortie. La liste des volumes refuse de même, et dit pourquoi, quand `pvesm status` ou le dernier stockage listé échoue ; un stockage en panne avant le dernier ne retire que ses propres volumes, qui ne sont jamais offerts. Un `pvesm status` en échec ne se lit plus comme « Rien d'orphelin ». La liste de la grappe reste lisible quand une bannière sshd ou un avertissement de ssh ou de sudo suit son document JSON
