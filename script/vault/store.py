...
class SecretStore:
    ...
    def _refuse_si_essai(self, verbe: str) -> None:
        """Un essai à blanc ne touche à AUCUN secret, pas même en lecture.

        Le refus LÈVE et ne rend jamais None : un None se lit comme « ce
        secret n'existe pas », et l'appelant enchaînerait en le créant ou en
        déclarant le compte non configuré. Une annonce ne doit pas non plus
        déverrouiller un coffre, ce qui réclamerait la phrase de passe.
        """
        if self._dry_run:
            raise SecretError(f"{t('A dry run touches no secret')} ({verbe})")
