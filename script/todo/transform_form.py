...
def run_transform_form(ctx, run_app: bool = True):
    ...
    def brut(valeur):
        """Une chaîne rendue TELLE QUELLE, sans balisage console.

        Rich lit `[x]` comme une balise de style et l'AVALE : la case
        cochée n'apparaissait jamais, ni dans le tableau ni dans la
        légende, et l'écran perdait son seul retour visuel. `[ ]` et
        `[!]` survivaient, ce qui rendait le défaut illisible — la case
        vide s'affichait, la case cochée disparaissait.
        Ça ne concerne pas que les marques : la colonne des exemples
        porte des valeurs du fichier, et l'une qui contient des crochets
        serait mangée ou interprétée comme un style.
        """
    ...
    class Perimetre(App):
        ...
        def compose(self) -> ComposeResult:
            yield Header()
            # Le CHEMIN du fichier, qui peut porter des crochets.
            yield Static(
                brut("  %s  ·  %s" % (ctx["fichier"], ctx["format"])),
                id="tete",
            )
            with Horizontal():
            ...
            yield Static(brut("  %s" % t(TEXTE_LEGENDE)), id="legende")
            yield Footer()
        ...
        def on_mount(self) -> None:
            ...
            for feuille in ctx["feuilles"]:
                liste.add_option(
                    Option(
                        brut(self._resume_feuille(feuille)), id=feuille["nom"]
                    )
                )
        ...
        def _remplir(self):
            ...
            for colonne in feuille["colonnes"]:
                ...
                colonnes.add_row(
                    *(brut(c) for c in libelle_de_colonne(colonne, marque))
                )
            ...
            for ligne in feuille["lignes_sondees"]:
                sondees.add_row(
                    *(
                        brut(c)
                        for c in libelle_de_ligne_sondee(
                            ligne,
        ...
        def _rafraichir_les_feuilles(self):
            ...
            for autre in ctx["feuilles"]:
                liste.add_option(
                    Option(brut(self._resume_feuille(autre)), id=autre["nom"])
                )
            if rang is not None:
