...
def _lance_une_descente(pid):
    ...
    Par ARGUMENT, jamais par sous-chaîne. Un « pgrep -f deep_proxmox.py »
    posé dans une boucle de surveillance donne un shell dont la ligne de
    commande contient le motif, et un contrôle par sous-chaîne compte ce
    shell comme une descente. Un argument qui SE TERMINE par le nom du
    fichier, lui, ne peut venir que d'un interpréteur qu'on a lancé dessus.
    """
    try:
