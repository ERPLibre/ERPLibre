...
class TestPreviewNeverParsesTheMessageAsMarkup(TestComposeScreenMounted):
    ...
    async def _apercu(
        ...
        async with app.run_test() as pilot:
            ...
            app.current_ref = ref
            app.current_meta = lambda: meta
            app.session_for(self.account.name).syncer.fetch_body = (
                lambda f, u: brut
            )
            app.show_preview()
            await pilot.pause()
    ...
    async def test_a_bracket_token_in_the_body_is_shown_not_parsed(self):
        rendu = await self._apercu(corps=f"Bonjour {self.JETON} fin")
        self.assertIn("msg_token", rendu)

    async def test_a_bracket_token_in_the_subject_is_harmless(self):
        """Le sujet passait par la même chaîne de balisage que le corps."""
        rendu = await self._apercu(subject=f"Promo {self.JETON}")
    ...
    async def test_the_labels_are_still_emphasised(self):
        """Le contrôle qui empêche la correction paresseuse : tout rendre
        littéral en supprimant le gras passerait les tests ci-dessus."""
        from textual.widgets import Static
        ...
        async with app.run_test() as pilot:
            ...
            app.show_preview()
            await pilot.pause()
            contenu = app.query_one("#preview", Static).content
            styles = [str(s.style) for s in contenu.spans]
            self.assertTrue(
                any("bold" in s for s in styles),
                f"aucun libellé en gras : {styles}",
            )
