...
#
# Le nom compte. La valeur est un TÉMOIN, inventé, et sa présence en clair
# dans la transcription d'essai est le sujet même du test : sans elle, rien ne
# prouverait que le listage ne la recopie pas. L'appeler « secret » décrivait
# donc l'inverse de ce qu'elle est, et un analyseur qui juge sur le nom
# signalait une fixture comme un dépôt de données sensibles.
MARQUEUR = "marqueur-de-message-qui-ne-doit-pas-sortir"

TRANSCRIPT = [
    json.dumps({"type": "custom-title", "customTitle": MARQUEUR}) + "\n",
    json.dumps(
        {
            "type": "user",
            "cwd": CHEMIN,
            "gitBranch": "dev/essai",
            "message": {"role": "user", "content": MARQUEUR},
        }
    )
    + "\n",
]
    ...
    def test_aucun_contenu_de_message_ne_ressort(self):
        ...
        for session in sessions:
            for valeur in vars(session).values():
                self.assertNotIn(MARQUEUR, str(valeur))
