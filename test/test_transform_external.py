...
class TestMenuQuestions(unittest.TestCase):
    ...
    def test_le_chemin_complet_rend_les_douze_reponses(self):
        self._repondre(
            ...
            "",  # nombres, défaut oui
            "",  # calibre des chiffres, défaut non
            "",  # texte, défaut oui
            "",  # en-têtes, défaut non
            "42",  # graine
        )
    ...
    def test_le_chemin_COURT_ne_pose_que_quatre_questions(self):
        """Sept questions dont chacune a un défaut évident : les répondre
        une par une pour arriver au même endroit fait passer les invites
        par réflexe, et une invite qu'on passe ne consent à rien.
        """
        entrees = self._repondre("o", "", "", "n")
        options = self.menu._transform_ask_options(self.RAPPORT)
        self.assertEqual(len(entrees.demandes), 4)
        for cle, valeur in _tm().DEFAUTS_RAPIDES.items():
            with self.subTest(cle=cle):
                self.assertEqual(options[cle], valeur)

    def test_le_chemin_court_GARDE_les_largeurs(self):
        """Le seul choix qui ne suit pas le défaut de son invite. Ce que
        l'option abandonne — l'étendue mesurée — est dit par l'aperçu."""
        self._repondre("o", "", "", "n")
        options = self.menu._transform_ask_options(self.RAPPORT)
        self.assertTrue(options["calibre_chiffres"])

    def test_le_chemin_court_DIT_ce_qu_il_prend(self):
        """Un raccourci qui ne dit pas ce qu'il choisit n'est pas un
        raccourci mais une surprise."""
        self._repondre("o", "", "", "n")
        self.menu._transform_ask_options(self.RAPPORT)
        rendu = self.sortie.getvalue()
        self.assertIn(todo_i18n.t("Taken as given:"), rendu)
        self.assertIn(todo_i18n.t("every sheet"), rendu)
        self.assertIn(todo_i18n.t("numbers replaced, widths kept"), rendu)

    def test_le_chemin_court_pose_QUAND_MEME_macros_et_graphiques(self):
        """Elles ne se voient pas dans les cellules et ne se devinent
        pas : la réponse change ce que la copie porte."""
        entrees = self._repondre("o", "", "", "n", "o", "n")
        rapport = dict(
        ...
        self.assertEqual(len(entrees.demandes), 6)

    def test_le_chemin_court_n_efface_pas_les_reponses_de_l_ECRAN(self):
        """L'écran a pu répondre par feuille : refuser les options
        avancées ne doit pas effacer ce qu'on vient de cocher."""
        self.menu._transform_ecran = lambda *args: {
            "colonnes_intactes_index_par_feuille": {"Ventes": [2]},
            "entetes_par_feuille": {"Ventes": [1]},
        }
        self._repondre("o", "", "", "n")
        options = self.menu._transform_ask_options(self.RAPPORT)
        self.assertEqual(
            options["colonnes_intactes_index_par_feuille"], {"Ventes": [2]}
        )
        self.assertEqual(options["colonnes_intactes"], [])

    def test_le_resume_DIT_les_colonnes_gelees_a_l_ecran(self):
        """Le chemin court garde ce que l'écran a gelé : annoncer
        « aucune colonne laissée intacte » dirait le contraire de ce qui
        s'applique, sur le seul écran où l'opérateur consent."""
        self.menu._transform_ecran = lambda *args: {
            "colonnes_intactes_index_par_feuille": {"Ventes": [2, 5]},
            "entetes_par_feuille": {"Ventes": [1]},
        }
        self._repondre("o", "", "", "n")
        self.menu._transform_ask_options(self.RAPPORT)
        rendu = self.sortie.getvalue()
        self.assertIn(
            todo_i18n.t("column(s) left untouched, chosen on screen"), rendu
        )
        self.assertIn("2 ", rendu)
        self.assertNotIn(todo_i18n.t("no column left untouched"), rendu)

    def test_le_resume_dit_AUCUNE_quand_l_ecran_n_a_rien_gele(self):
        self._repondre("o", "", "", "n")
        self.menu._transform_ask_options(self.RAPPORT)
        rendu = self.sortie.getvalue()
        self.assertIn(todo_i18n.t("no column left untouched"), rendu)
        self.assertNotIn(
            todo_i18n.t("column(s) left untouched, chosen on screen"), rendu
        )

    def test_le_resume_DIT_la_feuille_que_l_ecran_a_privee_d_en_tete(self):
        """Un empan VIDE veut dire « pas d'en-tête », et l'écran y mène
        d'une frappe : la première ligne devient de la donnée et se fait
        remplacer. Annoncer l'en-tête protégé dit alors le contraire de
        ce qui s'applique."""
        self.menu._transform_ecran = lambda *args: {
            "colonnes_intactes_index_par_feuille": {},
            "entetes_par_feuille": {"Ventes": []},
        }
        self._repondre("o", "", "", "n")
        self.menu._transform_ask_options(self.RAPPORT)
        rendu = self.sortie.getvalue()
        self.assertIn(
            todo_i18n.t("sheet(s) with no header row on screen"), rendu
        )
        self.assertNotIn(todo_i18n.t("the header row is left readable"), rendu)

    def test_un_empan_declare_laisse_l_en_tete_protege(self):
        self.menu._transform_ecran = lambda *args: {
            "colonnes_intactes_index_par_feuille": {},
            "entetes_par_feuille": {"Ventes": [1]},
        }
        ...
        self.menu._transform_ask_options(self.RAPPORT)
        rendu = self.sortie.getvalue()
        self.assertIn(todo_i18n.t("the header row is left readable"), rendu)
        self.assertNotIn(
            todo_i18n.t("sheet(s) with no header row on screen"), rendu
        )

    def test_une_feuille_ABSENTE_de_la_table_garde_son_en_tete(self):
        """Absente veut dire « mesure-la », non « elle n'en a pas »."""
        self.menu._transform_ecran = lambda *args: {
            "colonnes_intactes_index_par_feuille": {},
            "entetes_par_feuille": {},
        }
    ...
    def test_les_macros_ne_se_demandent_que_si_le_fichier_en_a(self):
        ...
        self.assertTrue(options["garder_macros"])
        self.assertFalse(options["garder_graphiques"])
        self.assertEqual(len(entrees.demandes), 13)
...
class TestMenuDerouleComplet(unittest.TestCase):
    ...
    def test_refuser_l_apercu_n_appelle_PAS_apply(self):
        ...
        self._dialogue(
            self.source,
            "o",  # anonymiser
            "",  # convertir : garder le format
            "",  # table du lot
            "o",  # options avancées
            "",  # colonnes intactes
            "",  # nombres
            "",  # calibre des chiffres
            "",  # texte
            "",  # en-têtes
            "",  # graine
...
class TestEntreeInstallation(unittest.TestCase):
    """« Installer l'environnement de lecture » dit ce qui est là.

    Elle interrogeait le gestionnaire de paquets pour savoir si l'outil
    existait, et rendait son ignorance comme une absence : « aucun paquet
    connu » devant un `mdb-queries` installé.
    """
    ...
    def setUp(self):
        ...
        sys.stdout = self.sortie
        self.addCleanup(setattr, sys, "stdout", vrai)
        # Le venv de lecture est supposé prêt : la branche qui le BÂTIT
        # n'a rien à faire dans une suite unitaire.
        self._remplacer("available", lambda fmt=None: True)

    def _remplacer(self, nom, valeur):
        vrai = getattr(transform_setup, nom)
        setattr(transform_setup, nom, valeur)
        self.addCleanup(setattr, transform_setup, nom, vrai)

    def test_l_outil_present_est_ANNONCE(self):
        self._remplacer("requetes_access_lisibles", lambda: True)
        self._remplacer(
            "system_packages_cmd", lambda: ["ne-doit-pas-etre-appele"]
        )
        self.menu._transform_install_env()
        rendu = self.sortie.getvalue()
        self.assertIn(
            todo_i18n.t("Access saved queries are readable here."), rendu
        )
        self.assertNotIn("ne-doit-pas-etre-appele", rendu)

    def test_sans_outil_ni_paquet_connu_l_entree_dit_QUOI_FAIRE(self):
        """Le constat seul laissait l'opérateur sans suite."""
        self._remplacer("requetes_access_lisibles", lambda: False)
        self._remplacer("system_packages_cmd", lambda: None)
        self.menu._transform_install_env()
        rendu = self.sortie.getvalue()
        self.assertIn("mdb-queries", rendu)
        self.assertIn(
            todo_i18n.t("Install it by hand to read Access saved queries."),
            rendu,
        )
...
class TestCalibreDesChiffres(unittest.TestCase):
    """« Garder le nombre de chiffres » : 8839 tire dans 1000..9999.

    L'option échange l'étendue mesurée de la colonne contre celle des
    nombres de MÊME LARGEUR. Les deux ne tiennent pas ensemble — une
    colonne mêle des largeurs — et c'est l'opérateur qui tranche.
    """
    ...
    def _tirer(self, valeur, bornes=(1, 999999, True)):
        return noyau.nouveau_nombre(
            valeur, self.rng, bornes=bornes, table=self.table, calibre=True
        )

    def test_la_largeur_est_conservee(self):
        for valeur in (7, 42, 839, 8839, 12345, 987654):
            with self.subTest(valeur=valeur):
                rendu = self._tirer(valeur)
                self.assertEqual(
                    len(str(abs(int(rendu)))), len(str(abs(valeur)))
                )

    def test_le_signe_l_est_aussi(self):
        for valeur in (-7, -8839):
            with self.subTest(valeur=valeur):
                rendu = self._tirer(valeur)
                self.assertLess(rendu, 0)
                self.assertEqual(
                    len(str(abs(int(rendu)))), len(str(abs(valeur)))
                )

    def test_la_valeur_ne_se_rend_pas_a_elle_meme(self):
        for _ in range(30):
            self.assertNotEqual(self._tirer(8839), 8839)

    def test_sous_l_unite_l_etendue_MESUREE_l_emporte(self):
        """« Garder le nombre de chiffres » n'a pas de sens pour 0,15 :
        appliquer la règle quand même tirerait un taux entre 1 et 9, ce
        que l'étendue mesurée existe pour empêcher."""
        self.assertIsNone(noyau.bornes_du_meme_calibre(0.15))
        for _ in range(30):
            rendu = self._tirer(0.15, bornes=(0.1, 0.9, False))
            self.assertLess(rendu, 1)
            self.assertGreater(rendu, 0)

    def test_les_bornes_du_calibre(self):
        self.assertEqual(noyau.bornes_du_meme_calibre(8839), (1000, 9999))
        self.assertEqual(noyau.bornes_du_meme_calibre(7), (1, 9))
        self.assertEqual(noyau.bornes_du_meme_calibre(-42), (-99, -10))
        # La partie ENTIÈRE décide : un montant garde sa largeur.
        self.assertEqual(noyau.bornes_du_meme_calibre(1234.56), (1000, 9999))

    def test_sans_l_option_l_etendue_mesuree_gouverne(self):
        """La règle par défaut ne bouge pas : c'est elle qui empêche un
        taux de devenir 743."""
        rng = random.Random(1234)
        for _ in range(30):
            rendu = noyau.nouveau_nombre(
                8839, rng, bornes=(1, 20, True), table=noyau.Correspondance()
            )
            self.assertLessEqual(rendu, 20)

    def test_l_etendue_ABANDONNEE_est_annoncee(self):
        """L'option échange une garantie contre une autre : le dire est
        la moitié du travail. Sans l'avertissement, un taux qui sort de
        sa plage se découvre à la réimportation."""
        import tempfile

        base = tempfile.mkdtemp()
        self.addCleanup(shutil.rmtree, base, True)
        source = os.path.join(base, "m.csv")
        with open(source, "w", encoding="utf-8") as flux:
            flux.write("ref,montant\nA,8839\nB,9124\nC,7733\n")
        marqueur = "digit count"
        for calibre in (False, True):
            with self.subTest(calibre=calibre):
                apercu = formats.plan(
                    source,
                    {
                        "nombres": True,
                        ...
                        "feuilles": [],
                        "colonnes_intactes": [],
                        "calibre_chiffres": calibre,
                        "destination": os.path.join(base, "o.csv"),
                    },
                )
                dits = [a for a in apercu["avertissements"] if marqueur in a]
                self.assertEqual(bool(dits), calibre)

    def test_l_integrite_de_la_table_tient_toujours(self):
        """Même clé, même nombre — c'est ce qui garde les jointures."""
        premier = self._tirer(8839)
        self.assertEqual(self._tirer(8839), premier)

    def test_deux_valeurs_distinctes_ne_collisionnent_pas(self):
        rendus = {self._tirer(v) for v in range(1000, 1040)}
        self.assertEqual(len(rendus), 40)
...
class TestHorodatageDeLaCopie(unittest.TestCase):
    ...
    `dcterms:created` dit la séance de l'opérateur, que rien n'oblige à
    transmettre. Il rend surtout le refus du filet ALÉATOIRE : une
    cellule qui porte le mois courant — un libellé de période, ordinaire
    dans un export comptable — se retrouve dans l'horodatage, et le
    compte en surplus du socle bascule d'une exécution à l'autre selon
    que `created` et `modified` tombent sur la même seconde. Une entrée
    identique est alors refusée une fois sur deux ou trois.
    """
    ...
    def _source(self):
        """Un classeur dont une cellule porte le mois COURANT.

        Calculé et non écrit en dur : un littéral rendrait le test caduc
        au mois suivant, et c'est justement la coïncidence avec l'instant
        de l'écriture qui est éprouvée.
        """
        from openpyxl import Workbook

        chemin = os.path.join(self.base, "periode.xlsx")
        classeur = Workbook()
        onglet = classeur.active
        onglet.append(["periode", "montant"])
        mois = datetime.datetime.now().strftime("%Y-%m")
        for rang in range(4):
            onglet.append([mois, 100 + rang])
        classeur.save(chemin)
        return chemin, mois

    def test_le_mois_courant_dans_une_cellule_n_empeche_pas_l_ecriture(self):
        """Trois passes : sans le gel, une entrée identique est
        refusée une fois sur deux."""
        source, _mois = self._source()
        for passe in range(3):
            with self.subTest(passe=passe):
                sortie = os.path.join(self.base, "c%d.xlsx" % passe)
                _ecrire_par_sous_processus(source, sortie, {"graine": "5"})
                self.assertTrue(os.path.isfile(sortie))

    def test_la_copie_porte_une_date_de_creation_FIGEE(self):
        source, _mois = self._source()
        sortie = os.path.join(self.base, "c.xlsx")
        _ecrire_par_sous_processus(source, sortie, {"graine": "5"})
        with zipfile.ZipFile(sortie) as archive:
            core = archive.read("docProps/core.xml").decode("utf-8")
        self.assertIn(formats.EPOQUE_FIGEE.strftime("%Y-%m-%d"), core)

    def test_le_createur_ne_dit_pas_openpyxl(self):
        """Sans `creator=None`, l'élément n'est pas retiré mais REMPLI."""
        source, _mois = self._source()
        sortie = os.path.join(self.base, "c.xlsx")
        _ecrire_par_sous_processus(source, sortie, {"graine": "5"})
        with zipfile.ZipFile(sortie) as archive:
            core = archive.read("docProps/core.xml").decode("utf-8")
        self.assertNotIn("openpyxl", core)


class TestLaBandeDuCalibreNeSElargitPas(unittest.TestCase):
    """Garder la largeur : le vivier sature avant la bande.

    Sans remise, neuf valeurs à un chiffre se disputent les neuf places
    1..9 et l'identité est interdite : l'attribution bute. Hors calibre,
    trois recours élargissent alors la plage — d'un pas, par paliers de
    dix, puis en rendant « valeur + 1 » — et chacun rend un nombre d'une
    AUTRE largeur, la seule chose que l'option promette. Sous calibre les
    trois cèdent la place à un doublon dans la bande.

    Aucun avertissement ne couvre ce cas : les colonnes saturées se
    mesurent sur l'étendue de la COLONNE, pas sur la bande du calibre.
    """

    VALEURS = list(range(1, 10)) + [500000]
    BORNES = (1, 500000, True)

    def _largeur(self, n):
        return len(str(abs(int(n))))

    def _lot(self, graine, valeurs=None, bornes=None, calibre=True):
        rng = random.Random(graine)
        table = noyau.Correspondance()
        return {
            v: noyau.nouveau_nombre(
                v,
                rng,
                bornes=bornes or self.BORNES,
                table=table,
                calibre=calibre,
            )
            for v in (valeurs or self.VALEURS)
        }

    def test_aucune_graine_ne_change_une_largeur(self):
        """Sans la garde, 24 graines sur 300 en changent une."""
        fautifs = []
        for graine in range(300):
            for source, sortie in self._lot(graine).items():
                if self._largeur(sortie) != self._largeur(source):
                    fautifs.append((graine, source, sortie))
        self.assertEqual([], fautifs[:5])

    def test_la_bande_saturee_rend_un_DOUBLON_et_non_une_largeur(self):
        """Un doublon ne fait rien fuir ; une largeur changée se voit."""
        sorties = self._lot(1)
        chiffres = [s for v, s in sorties.items() if v < 10]
        self.assertTrue(all(1 <= s <= 9 for s in chiffres), chiffres)
        self.assertLess(len(set(chiffres)), len(chiffres))

    def test_jamais_l_identite_meme_saturee(self):
        """La rendre laisserait l'original dans la copie en le comptant
        comme remplacé — une fuite annoncée propre."""
        for graine in range(200):
            for source, sortie in self._lot(graine).items():
                self.assertNotEqual(source, sortie, (graine, source))

    def test_l_unicite_tient_encore_quand_la_bande_a_de_la_place(self):
        """Le compromis ne se paie que sur une bande pleine."""
        valeurs = [1000, 2000, 3000, 4000]
        sorties = self._lot(7, valeurs=valeurs, bornes=(1, 500000, True))
        self.assertEqual(len(valeurs), len(set(sorties.values())))
        for source, sortie in sorties.items():
            self.assertEqual(4, self._largeur(sortie), (source, sortie))

    def _saturer(self, valeurs):
        """Une table dont TOUTE la bande de ces valeurs est déjà prise.

        C'est la seule façon d'atteindre le recours : sans saturation, le
        tirage trouve une place libre et le test passe aussi bien sur le
        code d'avant.
        """
        table = noyau.Correspondance()
        for valeur in valeurs:
            bas, haut = noyau.bornes_du_meme_calibre(valeur)
            table.nombres_pris.update(range(bas, haut + 1))
        return table

    def test_le_signe_survit_a_la_bande_VRAIMENT_saturee(self):
        table = self._saturer([-5])
        rng = random.Random(3)
        for tour in range(50):
            sortie = noyau.nouveau_nombre(
                -5, rng, bornes=(-500000, -1, True), table=table, calibre=True
            )
            self.assertLessEqual(sortie, -1, tour)
            self.assertGreaterEqual(sortie, -9, tour)
            self.assertNotEqual(-5, sortie, tour)

    def test_la_bande_saturee_ne_rend_JAMAIS_l_identite(self):
        """La rendre laisserait l'original dans la copie en le comptant
        pour remplacé — une fuite annoncée propre."""
        for valeur in (1, 5, 9, -9, 42, -1234, 987654):
            with self.subTest(valeur=valeur):
                table = self._saturer([valeur])
                rng = random.Random(valeur)
                for _ in range(30):
                    sortie = noyau.nouveau_nombre(
                        valeur,
                        rng,
                        bornes=(-(10**7), 10**7, True),
                        table=table,
                        calibre=True,
                    )
                    self.assertNotEqual(valeur, sortie)
                    self.assertEqual(
                        self._largeur(valeur), self._largeur(sortie)
                    )

    def test_la_sonde_a_trois_candidats_tranche_quand_le_tirage_boucle(self):
        """Un tirage peut retomber dix fois sur l'origine : les bornes
        tranchent alors à coup sûr, sans parcourir la bande."""
        rng = random.Random(0)
        rng.randint = lambda bas, haut: 5  # tout tirage rend l'origine
        sortie = noyau._doublon_du_meme_calibre(5, rng, 1, 9, True, 2)
        self.assertNotEqual(5, sortie)
        self.assertIn(sortie, (1, 9, 2))

    def test_sous_l_unite_l_elargissement_reste_PERMIS(self):
        """Aucune bande ne s'applique sous l'unité : la règle générale
        garde son dernier recours, et il SORT de l'étendue.

        L'étendue 0,10..0,12 à deux décimales ne compte que trois places :
        cinq valeurs distinctes la saturent. La résolution se lit sur les
        valeurs, si bien qu'un 0,153 en porterait trois et ouvrirait
        cinquante-et-une places — la plage ne saturerait plus, et le test
        ...
        sorties = [
            ...
            for valeur in (0.10, 0.11, 0.12, 0.13, 0.14)
        ]
        self.assertTrue(
            any(s < 0.10 or s > 0.12 for s in sorties),
            "aucune sortie n'a quitté l'étendue : rien n'a été élargi",
        )

    def test_sans_calibre_l_elargissement_SORT_de_la_plage(self):
        """L'option seule interdit d'élargir ; la règle générale, non.

        Il faut PLUS de valeurs distinctes que de places : redemander une
        valeur déjà vue rend la sortie mémorisée, et neuf valeurs pour
        neuf places se rangent en permutation sans jamais saturer.
        """
        rng = random.Random(11)
        table = noyau.Correspondance()
        sorties = [
            noyau.nouveau_nombre(
                v, rng, bornes=(1, 9, True), table=table, calibre=False
            )
            for v in range(1, 15)
        ]
        self.assertTrue(
            any(s > 9 or s < 1 for s in sorties),
            "la plage 1..9 est saturée : le recours général doit en sortir",
        )

    def test_une_bande_compte_toujours_au_moins_neuf_places(self):
        """L'invariant sur lequel repose la sonde à trois candidats.

        La borne monte à vingt chiffres : c'est à seize que `float()`
        cessait d'être exact, et s'arrêter à quinze laissait justement le
        défaut hors du test.
        """
        for largeur in range(1, 21):
            valeur = 10 ** (largeur - 1)
            bas, haut = noyau.bornes_du_meme_calibre(valeur)
            self.assertGreaterEqual(haut - bas + 1, 9, largeur)
            self.assertEqual(largeur, len(str(haut)), largeur)
            self.assertEqual(largeur, len(str(bas)), largeur)

    def test_la_borne_haute_de_la_bande_reste_EXACTE(self):
        """`float(10**16 - 1)` remonte à 10**16, et `int()` le garde : la
        bande gagnait un chiffre.

        Le tirage est FORCÉ à sa borne haute. Un tirage ordinaire ne
        rencontrerait l'écart qu'une fois sur neuf mille milliards, si
        bien qu'un test statistique passerait des deux côtés sans rien
        éprouver — et les bornes vues sont assertées, non la seule sortie.
        """
        for largeur in range(14, 21):
            with self.subTest(largeur=largeur):
                ...
                self.assertEqual(
                    [(10 ** (largeur - 1), 10**largeur - 1)], vues
                )
                self.assertEqual(largeur, self._largeur(sortie), sortie)


class TestUnEntierPlusGrandQuUnFlottant(unittest.TestCase):
    """Un `.json` porte des entiers exacts, sans plafond de tableur.

    `json.loads` rend un entier de plusieurs centaines de chiffres
    intact, et `float()` le refuse au-delà de 1,8e308. Le tirage levait
    alors `OverflowError`, que le rattrapage général annonce comme un
    format inconnu : le fichier n'était pas anonymisé, et la valeur
    repartait en clair.

    Excel s'arrête à 9,99e307, donc aucun classeur n'y mène — mais
    `json.loads` n'a pas de plafond, et c'est un format que l'entrée
    accepte.
    """

    ENORME = int("9" * 400)

    def _nombre(self, valeur, bornes, **kw):
        return noyau.nouveau_nombre(
            valeur,
            random.Random(0),
            bornes=bornes,
    ...
    def test_un_json_porte_bien_un_entier_exact_de_400_chiffres(self):
        """La prémisse : sans elle le test ne prouve rien."""
        lu = json.loads('{"x": %s}' % ("9" * 400))["x"]
        self.assertIsInstance(lu, int)
        self.assertEqual(400, len(str(lu)))
        with self.assertRaises(OverflowError):
            float(lu)

    def test_une_etendue_trop_grande_ne_leve_plus(self):
        sortie = self._nombre(12345, (1, self.ENORME, True))
        self.assertNotEqual(12345, sortie)

    def test_la_valeur_elle_meme_est_REMPLACEE(self):
        """Lever laissait la valeur en clair dans la copie."""
        sortie = self._nombre(self.ENORME, (1, self.ENORME, True))
        self.assertNotEqual(self.ENORME, sortie)

    def test_le_calibre_garde_les_quatre_cents_chiffres(self):
        """Sa bande est calculée en entiers exacts : elle ne passe pas
        par le ramenage, et la largeur survit."""
        sortie = self._nombre(
            self.ENORME, (1, self.ENORME, True), calibre=True
        )
        self.assertEqual(400, len(str(abs(int(sortie)))))
        self.assertNotEqual(self.ENORME, sortie)

    def test_un_decimal_dans_une_telle_colonne_ne_leve_pas(self):
        sortie = self._nombre(1.5, (0.5, self.ENORME))
        self.assertNotEqual(1.5, sortie)

    def test_la_borne_ramenee_garde_son_SIGNE(self):
        self.assertEqual(sys.float_info.max, noyau._en_flottant(self.ENORME))
        self.assertEqual(-sys.float_info.max, noyau._en_flottant(-self.ENORME))
        self.assertEqual(12.5, noyau._en_flottant(12.5))

    def test_un_entier_hors_flottant_n_a_pas_de_decimale(self):
        self.assertEqual(0, noyau._decimales(self.ENORME))
        self.assertEqual(2, noyau._decimales(0.15))

    def test_une_bande_entiere_trop_large_se_compte_comme_infinie(self):
        """Le parcours des places est alors hors de portée, comme pour
        une étendue qui rendait déjà `inf`."""
        sortie = noyau._tirer_libre(
            self.ENORME,
            random.Random(0),
            10**399,
            10**400 - 1,
        ...
        self.assertEqual(400, len(str(sortie)))
        self.assertNotEqual(self.ENORME, sortie)
