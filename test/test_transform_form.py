...
class TestPilotage(unittest.TestCase):
    ...
    def _rendu(self, app):
        """Ce que l'écran AFFICHE, non ce que le modèle porte.

        Tous les autres pilotages lisent `app._intactes` ou une cellule
        du modèle. C'est ce qui a laissé passer un défaut d'AFFICHAGE
        complet : Rich lit `[x]` comme une balise de style et l'avale,
        donc la case cochée n'apparaissait jamais — ni dans le tableau,
        ni dans la légende — alors que le modèle disait « cochée ».
        """
        return "\n".join(
            "".join(segment.text for segment in bande)
            for bande in app.screen._compositor.render_strips()
        )

    def _ligne_rendue(self, app, repere):
        """La ligne AFFICHÉE qui porte ce repère.

        Chercher la marque dans tout l'écran ne prouverait rien : la
        légende en porte une, et la ligne d'en-tête mesurée est
        pré-cochée dans l'autre tableau.
        """
        for ligne in self._rendu(app).splitlines():
            if repere in ligne:
                return ligne
        return ""

    def test_la_case_cochee_APPARAIT_a_l_ecran(self):
        vu = {}
        ...
        async def scenario(app, pilote):
            ...
            table.focus()
            await pilote.pause()
            vu["avant"] = self._ligne_rendue(app, "montant")
            await pilote.press("space")
            await pilote.pause()
            vu["apres"] = self._ligne_rendue(app, "montant")

        self._piloter(scenario, taille=(100, 30))
        self.assertIn(transform_form.MARQUE_EN_PORTEE, vu["avant"])
        self.assertIn(transform_form.MARQUE_INTACTE, vu["apres"])
        self.assertNotIn(transform_form.MARQUE_EN_PORTEE, vu["apres"])

    def test_la_legende_montre_CHAQUE_marque_a_l_ecran(self):
        """La légende traversait le même filtre : son `[x]` disparaissait,
        donc l'opérateur ne pouvait pas non plus apprendre la marque."""
        vu = {}

        async def scenario(app, pilote):
            vu["texte"] = self._rendu(app)

        self._piloter(scenario, taille=(100, 30))
        for nom in ("MARQUE_INTACTE", "MARQUE_PLANCHER", "MARQUE_CORRIGEE"):
            with self.subTest(marque=nom):
                self.assertIn(getattr(transform_form, nom), vu["texte"])

    def test_une_valeur_a_crochets_n_est_pas_mangee(self):
        """La colonne des exemples porte des valeurs du FICHIER : l'une
        qui ressemble à une balise ne doit pas disparaître."""
        vu = {}
        rap = rapport()
        rap["feuilles"][0]["colonnes"][0]["exemples"] = ["[x]", "[bold]", "a"]

        app = transform_form.run_transform_form(
            transform_form.contexte_depuis_rapport(rap), run_app=False
        )
